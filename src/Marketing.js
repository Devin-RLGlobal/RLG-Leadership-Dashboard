...
const PriorityChip = styled(Chip)(({ priority }) => ({
  ...
  fontWeight: "bold",
  fontSize: "0.85rem",
  color: priority === "External Marketing" ? "#d32f2f" : priority === "Fundamentals" ? "#ff9800" : "#388e3c",
  backgroundColor: priority === "External Marketing" ? "#fce4e4" : priority === "Fundamentals" ? "#fff3e0" : "#e8f5e9",
}));
...
function TaskBoard() {
  ...
  const fetchProjects = async (currentSkip) => {
    try {
      const response = await axios.get(`https://sea-lion-app-hcfn5.ondigitalocean.app/todos?skip=${currentSkip}`);
